...
torch::Tensor get_scale_impl(torch::Tensor &in) {
  auto scale =
      torch::zeros({1}, torch::dtype(torch::kFloat32).device(in.device()));
  // get scale: t/127
  kernelErrchk();
  float *in_ptr = in.data_ptr<float>();
  float *out_ptr = scale.data_ptr<float>();
  if (((size_t)in_ptr & 0xfff) == 0) {
    kernel_get_scale<<<12 * 80, 128>>>(out_ptr, in_ptr, in.numel());
  } else {
    kernel_get_scale_float1<<<12 * 80, 128>>>(out_ptr, in_ptr, in.numel());
  }
  kernelErrchk();
  return scale;
}
...
#define call_transpose_quant(n)                                                \
  {                                                                            \
    ...
    dim3 block((nrow + TILE_X - 1) / TILE_X, (ncol + TILE_Y - 1) / TILE_Y);    \
...
torch::Tensor transpose_quant_impl(torch::Tensor &in, torch::Tensor &scale,
  ...
  auto out =
      torch::empty({ncol, nrow}, torch::dtype(torch::kInt8).device(device));
